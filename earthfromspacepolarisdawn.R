# Earth seen from space: Moon, Polaris Dawn and ISS
# www.overfitting.net
...
dobserver.polarisdawn=1400.7  # SpaceX Polaris Dawn apogee (km)
...
DISTANCES=c(dobserver.iss, dobserver.polarisdawn, dobserver.moon)
NIMG=length(DISTANCES)  # number of images
...
for (d in DISTANCES) {
    ...
    # Polaris Dawn FOV: 110.1º diagonal -> XXXmm FF
...
img=NewBitmap(IMAGESIZE, IMAGESIZE)
TH=0.7
img=DrawCircle(img, NCOLDIV2, NROWDIV2, (IMAGESIZE/2)*(Rearth/Rearth)*TH,
               fill=TRUE, val=0.25)
img=DrawCircle(img, NCOLDIV2, NROWDIV2, (IMAGESIZE/2)*((Rearth+dobserver.iss)/Rearth)*TH)
img=DrawCircle(img, NCOLDIV2, NROWDIV2, (IMAGESIZE/2)*((Rearth+dobserver.polarisdawn)/Rearth)*TH)
SaveBitmap(img, "orbits.png")
